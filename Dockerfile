FROM centos:centos7
LABEL maintainer "Francisco Pedernera"

# ZABBIX_VERSION=trunk tags/6.0 branches/6.0

ENV \
  ZABBIX_VERSION=tags/6.0 \
  ZS_enabled=true \
  ZA_enabled=true \
  ZW_enabled=true \
  ZJ_enabled=false \
...
RUN \
  ...
  yum install -y tar git gcc automake make nmap traceroute iptstate wget \
              net-snmp-devel net-snmp-libs net-snmp net-snmp-perl iksemel \
              net-snmp-python net-snmp-utils java-1.8.0-openjdk \
              java-1.8.0-openjdk-devel postgresql-devel libxml2-devel gettext \
 ...
 `# reinstall glibc for locales` \
  ...
  git clone https://git.zabbix.com/scm/zbx/zabbix.git /usr/local/src/zabbix && \
  ...
  zabbix_revision=$(git info https://git.zabbix.com/scm/zbx/zabbix.git |grep "Last Changed Rev"|awk '{print $4;}') && \
  ...
  rm -rf fping-5.1 && \
  rm -rf fping-5.1tar.gz && \
  cd /usr/local/src/zabbix/frontends/php && \
  locale/make_mo.sh && \
  yum autoremove -y gettext git gcc automake postgresql-devel \
  ...
  rm -rf /usr/local/src/zabbix/[a,A,b,c,C,i,I,m,M,n,N,r,R,s,t,u,r,\.]* /usr/local/src/zabbix/depcomp /usr/local/src/zabbix/.git && \
...
RUN \
  ...
  yum clean all && \
  ### branding / menu ### && \
  sed -i "s/ZABBIX_VERSION.*'\(.*\)'/ZABBIX_VERSION', '\1 CTL'/g" /usr/local/src/zabbix/frontends/php/include/defines.inc.php && \
  grep 'XXL' /usr/local/src/zabbix/frontends/php/include/defines.inc.php && \
  sed -i "s#]))->addClass(ZBX_STYLE_FOOTER);#,' / ',(new CLink('CTL Information Technology', 'https://www.ctl.com.ar'))->addClass(ZBX_STYLE_GREY)->addClass(ZBX_STYLE_LINK_ALT)->setAttribute('target', '_blank'),]))->addClass(ZBX_STYLE_FOOTER);#g" /usr/local/src/zabbix/frontends/php/include/html.inc.php && \
  grep 'www.ctl.com.ar' /usr/local/src/zabbix/frontends/php/include/html.inc.php && \
  sed -i "s/(new CDiv())->addClass(ZBX_STYLE_SIGNIN_LOGO),/(new CDiv())->addClass(ZBX_STYLE_SIGNIN_LOGO),(new CDiv())->addClass(ZBX_STYLE_CENTER)->addItem((new CTag('h1', true, _('Zabbix CTL')))->addClass(ZBX_STYLE_BLUE))->addItem((new CLink(_('Maintained by Francisco Pedernera'), 'http:\/\/www.ctl.com.ar'))->setTarget('_blank')->addClass(ZBX_STYLE_GREY)),/g" /usr/local/src/zabbix/frontends/php/include/views/general.login.php && \
  grep 'www.ctl.com.ar' /usr/local/src/zabbix/frontends/php/include/views/general.login.php && \
